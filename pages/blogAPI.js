...
const Insights = () => {
  ...
  const [insightsFetch, setInsightsFetch] = useState([]);
  ...
  const [loaderMessage, setLoaderMessage] = useState("Welcome to REKIT Blog");
  ...
  const pagination = (arr) => {
    let paginatedArr = [];
    let div = arr.length / 10;
    let mod = arr.length % 10;
    let pages = mod == 0 ? div : ++div;
    let j = 0;
    for (let i = 1; i <= pages; i++) {
      paginatedArr.push(arr.slice(j, j + 10));
      j = j + 10;
    }
    return paginatedArr;
  };
  ...
  const postHandler = (event) => {
    ...
    setPost(insightsFetch.find(({ id }) => id === +postid));
    setNextPage(false);
    setPrevPage(false);
    setRevealPost(true);
    setEffectLoad(false);
  ...
  useEffect(() => {
    ref.current.focus();
    axios
      .get("http://localhost:8080/rekit/InsightsAPI")
      .then(({ data }) => {
        if (data.status) {
          setViewPost(sliceToTen(data.data));
          setInsightsFetch(data.data);
          data.data.length && setEffectLoad(true);
          data.data.length > page && setNextPage(true);
          setDisplayLoader(false);
          setBackgroundPost("white");
          setPaginatedPosts(pagination(data.data));
          setLoaderMessage("View all our Recent Blog Posts");
          setLoaderBtnMsg("Read More");
        } else setDisplayLoader(false);
      })
      .catch((err) => {
        setDisplayLoader(false);
        setLoaderMessage(
          "We couldnt fetch the Blog Posts. Please try again some other time"
        );
        console.error(err);
      });
  }, []);
  ...
  return (
    <div>
      ...
      <AppLayout>
        {/* {!displayLoader && (
          <div className="flex-col justify-center content-start lg:flex-row lg:justify-between mx-3 px-2">
            {categoryArr.map((category, idx) => {
              <div data-category={category} onClick={init} id={idx}>
                <p> {category}</p>
              </div>;
            })}
          </div>
        ...
        {displayLoader && (
          <div
            className="loader px-[0.5rem] lg:px-[3rem] lg:py-[3rem] mx-auto my-[5rem] w-full"
            ref={ref}
          ></div>
        )}
        <div className="flex-col justify-center content-center lg:flex-row md:flex-col">
          <div
            className={`w-full lg:w-${columnOneWidth} px-[0.5rem] lg:px-[2rem] pt-${paddingTop} mx-auto bg-${backgroundPost} pb-${paddingTop} border-r-${seperateBorder}`}
          >
            {
          ...
          <div
            className={`w-full lg:w-${columnTwoWidth} px-[0.5rem] lg:px-[2rem] pt-${paddingTop} mx-auto bg-${backgroundPost} pb-${paddingTop}`}
          >
            {effectLoad &&
              viewPost.map(
                (
                  { author, title, updatedDate, id, description, content },
                  index
                ) => {
                  return (
                    <div
                    ...
                    >
                      ...
                      >
                        <InsightCard
                          ...
                          date={updatedDate}
                          articleLink={`${id}`}
                          description={content.split("paragraph ")[1]}
                        />
                      </div>
                    </div>
                  );
